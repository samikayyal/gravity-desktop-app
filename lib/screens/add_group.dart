...
class GroupPlayer {
  ...
  bool isSibling;
  bool isMainSibling;
  Set<String> existingPlayerPhones =
      {}; // Track phone numbers from this existing player
...
class _AddGroupState extends ConsumerState<AddGroup> {
  ...
  final List<bool> phoneReadOnlyStates = [
    false
  ]; // Track which phone numbers are from existing players

  // Focus nodes for tab order
  final List<FocusNode> phoneFieldFocusNodes = [FocusNode()];
                    ...
                    children: [
                      FocusTraversalOrder(
                        order: NumericFocusOrder(
                            1000.0), // Phone numbers come after all players
                        child: PhoneNumberEntryCard(
                          title: "Phone Numbers (FOR THE WHOLE GROUP)",
                          controllers: phoneControllers,
                          focusNodes: phoneFieldFocusNodes,
                          isDisabled: false, // Remove global disable
                          isPhoneReadOnly:
                              phoneReadOnlyStates, // Pass individual read-only states
                          disableListModification: false,
                          addOnPressed: () {
                            setState(() {
                              phoneControllers.add(TextEditingController());
                              phoneFieldFocusNodes.add(FocusNode());
                              phoneReadOnlyStates
                                  .add(false); // New phones are editable
                            });
                          },
                          removeOnPressed: (int index) {
                            setState(() {
                              if (phoneControllers.length > 1) {
                                ...
                                phoneFieldFocusNodes[index].dispose();
                                phoneFieldFocusNodes.removeAt(index);
                                phoneReadOnlyStates.removeAt(
                                    index); // Remove corresponding read-only state
                              }
                            });
                          },
                          onReorder: (int oldIndex, int newIndex) {
                            setState(() {
                              ...
                              final focusNode =
                                  phoneFieldFocusNodes.removeAt(oldIndex);
                              phoneFieldFocusNodes.insert(newIndex, focusNode);
                              final readOnlyState =
                                  phoneReadOnlyStates.removeAt(oldIndex);
                              phoneReadOnlyStates.insert(newIndex,
                                  readOnlyState); // Reorder read-only states too
                            });
                          },
                        ),
                      ),
  ...
  void dispose() {
    ...
    for (final focusNode in phoneFieldFocusNodes) {
      focusNode.dispose();
    }
    phoneReadOnlyStates.clear(); // Clear read-only states list
    ...
    return MyCard(
      child: Column(
        ...
        children: [
          ...
          FocusTraversalOrder(
            order: NumericFocusOrder(
                2000.0), // Amount paid comes after phone numbers
            child: MyTextField(
          ...
          SizedBox(
            ...
            child: FocusTraversalOrder(
              order: NumericFocusOrder(3000.0), // Submit button comes last
                ...
                children: [
                  if (player.isSibling) ...[
                    Expanded(
                      flex: 2,
                      child: FocusTraversalOrder(
                        order: NumericFocusOrder(index * 10 + 1.0),
                        child: MyTextField(
                          controller: player.firstNameController,
                          focusNode: player.firstNameFocusNode,
                    ...
                    Expanded(
                      flex: 2,
                      child: FocusTraversalOrder(
                        order: NumericFocusOrder(index * 10 + 2.0),
                        child: MyTextField(
                          controller: player.lastNameController,
                          focusNode: player.lastNameFocusNode,
                      ...
                      child: FocusTraversalOrder(
                        ...
                        child: Autocomplete<Player>(
                            ...
                            onSelected: (Player selection) async {
                              ...
                              setState(() {
                                // if last player switch focus to phone
                                // else switch to next player
                                if (index == groupPlayers.length - 1) {
                                  phoneFieldFocusNodes[0].requestFocus();
                                } else if (groupPlayers[index + 1].isSibling) {
                                  groupPlayers[index + 1]
                                      .firstNameFocusNode
                                      .requestFocus();
                                } else {
                                  groupPlayers[index + 1]
                            ...
                            fieldViewBuilder: (context, controller, focusNode,
                                onFieldSubmitted) {
                              // Use the player's focus node instead of the one from the callback
                              player.fullNameFocusNode.addListener(() {
                                if (player.fullNameFocusNode.hasFocus) {
                                          ...
                                          child: IconButton(
                                            ...
                                            onPressed: () async {
                                              setState(() {
                                                ...
                                                player.existingPlayerPhones
                                                    .clear(); // Clear tracked phone numbers
                                                _updatePhoneReadOnlyStates(); // Update phone read-only states after clearing player
                                              });
                                            },
                                          ),
                                        )
                  ...
                  Expanded(
                    flex: 1,
                    child: FocusTraversalOrder(
                      order: NumericFocusOrder(index * 10 + 3.0),
                      child: MyTextField(
                        controller: player.ageController,
                        focusNode: player.ageFocusNode,
  ...
  Future<void> _fillPlayerDetails(Player selection, int index) async {
    ...
    setState(() {
      ...
      // Track phones from this existing player
      player.existingPlayerPhones.clear();
      for (var playerPhone in playerPhones) {
        if (playerPhone.number.isNotEmpty) {
          player.existingPlayerPhones.add(playerPhone.number);
        }
      }
      ...
      for (var playerPhone in playerPhones) {
        ...
        if (!phoneControllers
          ...
          if (emptyControllerIndex != -1) {
            ...
            // Mark this phone as read-only since it belongs to an existing player
            if (emptyControllerIndex < phoneReadOnlyStates.length) {
              phoneReadOnlyStates[emptyControllerIndex] = true;
            }
          } else {
            ...
            phoneFieldFocusNodes.add(FocusNode());
            phoneReadOnlyStates
                .add(true); // Mark as read-only since it's from existing player
          }
        }
      }
  ...
  void _updatePhoneReadOnlyStates() {
    // Get all phone numbers from existing players
    Set<String> allExistingPlayerPhones = {};
    for (var player in groupPlayers) {
      if (player.existingPlayer != null) {
        allExistingPlayerPhones.addAll(player.existingPlayerPhones);
      }
    }

    // Update read-only states based on which phones belong to existing players
    for (int i = 0;
        i < phoneControllers.length && i < phoneReadOnlyStates.length;
        i++) {
      phoneReadOnlyStates[i] =
          allExistingPlayerPhones.contains(phoneControllers[i].text);
    }
  }
